...
"""
A gated recurrent model for classifying light curves.
...
class AstroGRUModel(astro_model.AstroModel):
    ...
    """
    super(AstroGRUModel, self).__init__(features, labels, hparams, mode)
    
    def build_gru_layers(self, inputs, hparams, scope="gru"):
    """ Builds GRU layers and the embedding layer.
    
    Args:
      inputs: A Tensor of shape [batch_size, length].
      hparams: Object containing GRU hyperparameters.
      scope: Name of the variable scope.

    Returns:
        A Tensor of shape [batch_size, output_size]
    """
        pass # TODO: set up architecture to build the GRU layers & Embedding layer
